...
func (m *MonitorZone) IsCalibrationExpired(now time.Time) bool {
	return !m.Equipment.CalibrationDue.IsZero() && now.After(m.Equipment.CalibrationDue)
}
...
func (m *MonitorZone) EffectiveEnvRange(process ProcessType) EnvRange {
	pt := ProcessDefaultsFor(process)
	out := EnvRange{
		TempMin: pt.TempMin, TempMax: pt.TempMax,
		HumidityMin: pt.HumidityMin, HumidityMax: pt.HumidityMax,
		PressureMin: pt.PressureMin, PressureMax: pt.PressureMax,
	}
